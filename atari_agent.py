...
class AtariAgent(parl.Agent):
    def __init__(self, algorithm, obs_dim, act_dim,
        ...
        assert isinstance(obs_dim, (list, tuple))
    ...
    def build_program(self):
        ...
        with fluid.program_guard(self.sample_program):
            ...
            self.sample_actions, self.action_mean, self.action_std = self.alg.sample(obs)
        ...
        with fluid.program_guard(self.learn_program):
            ...
            mean = layers.data(
                name='mean', shape=[self.act_dim], dtype='float32')
            std = layers.data(
                name='std', shape=[self.act_dim], dtype='float32')
            ...
            self.learn_reader = fluid.layers.create_py_reader_by_data(
                capacity=32,
                feed_list=[
                    obs, actions, mean, std, rewards, dones, lr,
                    entropy_coeff
                ])
            ...
            vtrace_loss, kl = self.alg.learn(obs, actions, mean, std, rewards, dones, lr, entropy_coeff)
            self.learn_outputs = [
    ...
    def sample(self, obs_np):
        ...
        sample_actions, action_mean, action_std = self.fluid_executor.run(
            self.sample_program,
            feed={'obs': obs_np},
            fetch_list=[self.sample_actions, self.action_mean, self.action_std])
        return sample_actions, action_mean, action_std
